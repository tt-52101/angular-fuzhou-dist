import { WechatMenuAppSeviceServiceProxy, MenuMatchRule, GetWechatMenuForEditOutput, MenuFull_RootButton, CreateOrEditWechatMenuInput } from '@shared/service-proxies/service-proxies';
...
export class CreateOrEditWechatMenuComponent extends AppComponentBase implements OnInit {
  ...
  ngOnInit() {
    this.activatedRoute.params
      .subscribe((params) => {
        ...
        let currentTitle = this.l('EditWechatMenu') + ":" + this.appName;
  ...
  getMenuData(isRefresh: boolean = false) {
    ...
    this._wechatMenuService.getMenuForEdit(this.appId)
      ...
      .subscribe((result) => {
        ...
        this.allMemu.menu.button.forEach(item => {
  ...
  save(): void {
    this.loading = true;
    let input: CreateOrEditWechatMenuInput = new CreateOrEditWechatMenuInput();
   ...
   */
  private processSubmitMenuData(menuItem: IWechatMenuInfo, parentMenu?: MenuFull_RootButton): MenuFull_RootButton {
    let meunFull: MenuFull_RootButton = MenuFull_RootButton.fromJS(menuItem.origin);
