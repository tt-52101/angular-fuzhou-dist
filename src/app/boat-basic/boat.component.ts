...
import {BoatServiceProxy, PagedResultDtoOfBoatListDto, BoatListDto,GetBoatsInput } from '@shared/service-proxies/service-proxies';
...
export class  BoatComponent extends PagedListingComponentBase<BoatListDto>
implements OnInit {
  ...
  curmenupower=[]
	...
	*/
	createOrEdit(id?: number): void {
		this.modalHelper.static(CreateOrEditBoatComponent, { id: id })
		.subscribe(result => {
			if (result) {
				this.refresh();
			}
	...
	*/
	delete(entity: BoatListDto): void {
		...
		abp.message.confirm(
			...
			res => {
				if (res) {
					...
					this._boatService.batchDelete(ids).subscribe(() => {
						this.refreshGoFirstPage();
						this.notify.success(this.l('SuccessfullyDeleted'));
					});
				}
